...
def probePointMean(probe_point_id, lattice_y, currentConfig, boxSize):
    probe_point_status = []
    id = probe_point_id
    ly = lattice_y
    if boxSize == 3:
        probe_point_ids = [id-ly-1, id-ly, id-ly+1,
                           id-1, id, id+1, 
                           id+ly-1, id+ly, id+ly+1]    # プローブ点を中心にした9点のID
    if boxSize == 5:
    ...
    if boxSize == 7:
        probe_point_ids = [id-3*ly-3, id-3*ly-2, id-3*ly-1, id-3*ly, id-3*ly+1, id-3*ly+2, id-3*ly+3, 
                           id-2*ly-3, id-2*ly-2, id-2*ly-1, id-2*ly, id-2*ly+1, id-2*ly+2, id-2*ly+3, 
                           id-ly-3, id-ly-2, id-ly-1, id-ly, id-ly+1, id-ly+2, id-ly+3, 
                           id-3, id-2, id-1, id, id+1, id+2, id+3,
                           id+ly-3, id+ly-2, id+ly-1, id+ly, id+ly+1, id+ly+2, id+ly+3, 
                           id+2*ly-3, id+2*ly-2, id+2*ly-1, id+2*ly, id+2*ly+1, id+2*ly+2, id+2*ly+3, 
                           id+3*ly-3, id+3*ly-2, id+3*ly-1, id+3*ly, id+3*ly+1, id+3*ly+2, id+3*ly+3]    # プローブ点を中心にした49点のID
...
def drawStepEvolutionFunc(maxPoints, x_orig_list, y_orig_list):     # アニメーション用のリストのリストを作る
    ...
    x_array = np.asanyarray(x_list, dtype=object)
    y_array = np.asanyarray(y_list, dtype=object)    
